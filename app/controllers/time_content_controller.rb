class TimeContentController < ApplicationController
  ...
  def past_record
    ...
    @content_yesterday_graph_data = TimeContent.where(user_id: current_user, start_time: @yesterday).order('content_id ASC').content_data
    ...
    @content_last_week_graph_data = TimeContent.where(user_id: current_user, start_time: @last_week).order('content_id ASC').content_data
